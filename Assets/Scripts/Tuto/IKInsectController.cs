...
public class IKInsectController : MonoBehaviour
{
    ...
    public List<FootIKTarget> footIKTargets = new List<FootIKTarget>(6);// the targets for the position of the feet

    public Transform skeleton;//the root of the skeleton
    public Transform rig; //the parent of the IK targets

    private Vector3[] stableFeetPositions = new Vector3[6];//the position of the feet the last time they were touching the ground
    ...
    private void ForwardVectorFeet()
    {
        Debug.DrawLine(stableFeetPositions[1] + transform.forward, stableFeetPositions[1] + transform.forward * 10, Color.green);
        Debug.Log("Draw");
        Debug.Log(stableFeetPositions[1]);
    }
    ...
    private IEnumerator TakeSteps(float duration)
    {
        ...
        while (isMoving)
        {
            ...
            MoveFoot(4, Mathf.Clamp(nTime - 1, 0, 1));
            MoveFoot(5, Mathf.Clamp(nTime - delay - 1, 0, 1));
            ...
            if (nTime >= 2 + delay)
            {
                ...
                UpdateStableFootPosition(4);
                UpdateStableFootPosition(5);
                cycled = false;
            }
